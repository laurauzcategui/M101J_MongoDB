...
public class BlogPostDAO {
	...
	// changes to templates and post retrieval code.
        ...
        DBObject document = findByPermalink(permalink);
        postsCollection.save(document);
        List<DBObject> comments = (List<DBObject>) document.get("comments");

            DBObject comm = comments.get(ordinal);
            System.out.println(comm);
            Object obj = comm.get("num_likes");
            System.out.println(obj);
            int i = Integer.parseInt( obj.toString() )+1;
        comm.removeField("num_likes");
            comm.put("num_likes", i);
            System.out.println(comm);
        postsCollection.save(document);
            WriteResult result = postsCollection.update(new BasicDBObject("permalink", permalink),
                new BasicDBObject("$push", new BasicDBObject("comments", comm)),true,false);
